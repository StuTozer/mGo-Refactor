...
SUPPORTED_SHADERS = ['AiStandard', 'VRayMtl', 'RedshiftArchitectural']  # currently supported shaders
...
class MgoUI(QtGui.QWidget):
    """
    Main mGo functionality specific to Mari itself. Creates a Mari palette, setting the bodyWidget with a .ui file
    made in Qt Designer.

    This class handles export of all data (textures/shaders/geo) from Mari to other potential applications.


    TODO:   Export layered shaders
            Generate Tileble MipMaps on texture export (.tx support etc)
            Save mGo palette config data
            IP/Networking stuff
    """
    ...
    # output path for mGo shader/texture data
    ...
    # send commands to Maya or other 3d app - very simple at present (just does import function)
    def send_command_to_app(self):
        ...
        try:
            ...
            self.print_message("--- ALERT --- please open port 6010 in Maya first")
    ...
    # get rid of any bad characters and replace with underscore
...
"""
A few extra functions, which are called at times by 'mgo_app_ui.py', in order to do Mari specific things while
working inside a 3d application. Example- 'import_cameras' function is called by the 'send_cameras' method in
mgo_app_ui.py (at line 286), after the cameras have been exported as an .fbx to a specified folder.

Here we should be able to import cameras, plug HDRI's into Mari's Environment light, change or create new Mari projects,
etc. These functions would also import geo into Mari, set subdivision levels, etc (I don't have any new code related to
geo import in Mari currently).
"""

# makes a list of Mari projects and saves it in dict key ['mari projects'] to the user's 'mgo_config.txt' file
...
# imports a previously exported HDRI image, and plugs the texture into Mari's environment light
...
# switches to the desired Mari project
