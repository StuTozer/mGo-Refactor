...
# python code to open mGo UI in Maya...
"""
...
# thinking about a good place to put UIs
...
# returns the application the user is working in
def _get_app():
...
class MgoAppUI(QtGui.QWidget):
    ...
    Methods in this class deal with reading or writing data to the app configuration file, 'mgo_config.txt'
    ...
    During initialisation, the 'mgo_config.txt' file is written to the user's Mari/mGo directory
    ...
    def send_geo(self):
        ...
        export_hierarchy = self.ui.exportHierarchy_chk.isChecked()
        merge_hierarchy = self.ui.mergeHierarchy_chk.isChecked()
        tex_res = self.ui.chanRes_combo.currentText()
        subdivs = self.ui.subdivs_combo.currentText()

        geo = self.app.GeoExport(self.project, self.assets_path, self.send_mode, tex_res, subdivs, is_animated,
                           export_hierarchy, merge_hierarchy)

        self.mari_connect()
        if self.mari:
            print "ready to send commands to mari"
            print geo.output_paths

            # TODO: Mari geo import functions

    # renders and exports an HDRI of the current 3d scene, and plugs the exported texture into Mari's env light
    def send_hdri(self):
    ...
    # simple export cameras function
    ...
    # exports selected shaders as data stored in .pre format, for use with mGo's Materialiser in Mari
